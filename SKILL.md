...
> 📍 **地址與營業狀態一律以 Google Maps 為準**：當要確認一間店的**地址**，或**是否還在營業**
> （未歇業/搬遷/改名）時，以 Google Maps 上的最新資訊為主——社群貼文、部落格、舊截圖或使用者
> 記憶都可能過時。可用 `fetch_place.py` 帶該店的 Google Maps 連結重新核對；查不到對應的 Maps
> 條目時，先回報使用者「Google Maps 上查無此店／可能已歇業」，再決定是否入庫或更新。
...
### 3. 入庫前必查：營業狀態（強制，不可略過）

入庫**任何一間**店之前，都必須先確認它**目前仍在營業**（未永久歇業、未結束營業、
未搬遷、未改名）。這是硬性關卡，先驗證、後入庫——**不可「先入庫再補查」**：

- **以 Google Maps 為準**：查該店在 Google Maps 上的營業狀態。社群貼文、部落格、
  懶人包、舊截圖、使用者記憶都可能過時，**不能單憑這些就判定還在營業**。
- 從部落格／懶人包／IG 等二手來源抓到的店，**逐間查證**確認 open 後才入庫；
  查到**永久歇業／結束營業**就**不入庫**，並回報使用者。
- 查到**搬遷／改名／同址換店**時：改用正確的店名與地址（座標一併更新）再入庫，
  別把舊資料寫進去。
- 查不到對應的 Maps 條目時，回報「Google Maps 查無此店／可能已歇業」，
  由使用者決定是否仍要入庫。
- **批次入庫**時，這道查證要對 manifest 裡的**每一筆**都做，不能整批略過。
...
- **入庫前一律先查營業狀態**（見「入庫流程 3」），確認仍在營業才入庫——先驗證、後入庫，不可先入庫再補查。
- **地址與「是否還在營業」一律以 Google Maps 上的資訊為準**，其他來源（社群、部落格、舊截圖、記憶）僅供參考。
